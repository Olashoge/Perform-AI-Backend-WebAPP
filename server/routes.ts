...
export async function registerRoutes(
  ...
  app.get("/api/week-data", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const scheduledPlans = await storage.getScheduledPlans(userId);
      const workoutPlansRaw = await storage.getScheduledWorkoutPlans(userId);
      const dailyMeals = await storage.getDailyMealsByDateRange(userId, startStr, endStr);
      const dailyWorkouts = await storage.getDailyWorkoutsByDateRange(userId, startStr, endStr);
      const completions = await storage.getCompletionsByDateRange(userId, startStr, endStr);
      ...
      for (let i = 0; i < 7; i++) {
        ...
        for (const plan of scheduledPlans) {
          ...
          for (let j = 0; j < planJson.days.length; j++) {
            ...
            if (dd.toISOString().slice(0, 10) === dateStr) {
              const dayMeals = (planJson.days[j] as any)?.meals;
              if (dayMeals) {
                for (const [slot, meal] of Object.entries(dayMeals)) {
                  mealsForDay.push({ slot, meal, sourceType: "plan", sourceId: plan.id });
                }
              }
            }
          }
        ...
        for (const dm of dailyMeals) {
          if (dm.date !== dateStr || dm.status !== "ready" || !dm.planJson) continue;
          const meals = (dm.planJson as any)?.meals;
          if (meals) {
            for (const [slot, meal] of Object.entries(meals)) {
              mealsForDay.push({ slot, meal, sourceType: "daily_meal", sourceId: dm.id });
            }
          }
        }

        const workoutsForDay: any[] = [];
        for (const wp of workoutPlansRaw) {
          if (!wp.planStartDate || !wp.planJson) continue;
          const planJson = wp.planJson as WorkoutPlanOutput;
          for (let j = 0; j < planJson.days.length; j++) {
            const dd = new Date(wp.planStartDate + "T00:00:00");
            dd.setDate(dd.getDate() + j);
            if (dd.toISOString().slice(0, 10) === dateStr) {
              const day = planJson.days[j];
              if (day && day.isWorkoutDay !== false) {
                workoutsForDay.push({ session: day.session, sourceType: "workout", sourceId: wp.id, dayIndex: day.dayIndex });
              }
            }
          }
        }
        ...
        for (const dw of dailyWorkouts) {
          if (dw.date !== dateStr || dw.status !== "ready" || !dw.planJson) continue;
          workoutsForDay.push({ session: (dw.planJson as any), sourceType: "daily_workout", sourceId: dw.id });
        }
  ...
  app.get("/api/day-data/:date", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const scheduledPlans = await storage.getScheduledPlans(userId);
      const workoutPlansRaw = await storage.getScheduledWorkoutPlans(userId);
      const dailyMeals = await storage.getDailyMealsByDateRange(userId, dateStr, dateStr);
      const dailyWorkouts = await storage.getDailyWorkoutsByDateRange(userId, dateStr, dateStr);
      ...
      for (const plan of scheduledPlans) {
        ...
        for (let j = 0; j < planJson.days.length; j++) {
          ...
          if (dd.toISOString().slice(0, 10) === dateStr) {
            ...
            if (dayMeals) {
              for (const [slot, meal] of Object.entries(dayMeals)) {
                meals.push({ slot, meal, sourceType: "plan", sourceId: plan.id });
              }
            }
          }
        }
      ...
      for (const dm of dailyMeals) {
        ...
        if (dmMeals) {
          for (const [slot, meal] of Object.entries(dmMeals)) {
            meals.push({ slot, meal, sourceType: "daily_meal", sourceId: dm.id });
          }
        }
      }
      ...
      for (const wp of workoutPlansRaw) {
        ...
        for (let j = 0; j < planJson.days.length; j++) {
          ...
          if (dd.toISOString().slice(0, 10) === dateStr) {
            const day = planJson.days[j];
            if (day && day.isWorkoutDay !== false) {
              workouts.push({ session: day.session, sourceType: "workout", sourceId: wp.id, dayIndex: day.dayIndex });
            }
          }
        }
      }
      ...
      for (const dw of dailyWorkouts) {
        if (dw.status !== "ready" || !dw.planJson) continue;
        workouts.push({ session: (dw.planJson as any), sourceType: "daily_workout", sourceId: dw.id });
      }
