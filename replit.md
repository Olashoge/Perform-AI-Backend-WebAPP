...
**Dev Database Setup**: The Replit platform-injected `DATABASE_URL` points to a legacy `helium` hostname that is unresolvable. A local PostgreSQL 16 instance is started by `start-postgres.sh` before the app launches (via the "Start application" workflow: `bash start-postgres.sh && npm run dev`). The data directory is at `.pgdata/` (gitignored). Both `server/db.ts` and `drizzle.config.ts` detect the broken helium URL and automatically remap it to `postgresql://postgres@localhost:5432/heliumdb`. Running `npm run db:push` works automatically. If the dev database needs to be rebuilt, delete `.pgdata/` and restart the workflow.

**Account Identity**: The `users` table stores `id`, `email`, `firstName` (nullable), `passwordHash`, `createdAt`, and `updatedAt`. The `firstName` field is the canonical display name. Signup collects `firstName`. All auth responses (`/api/auth/me`, `/api/auth/login`, `/api/auth/signup`, `/api/auth/token-login`) return `{id, email, firstName}`. Account management endpoints: `PATCH /api/account` (update firstName/email), `POST /api/account/change-password` (requires currentPassword + newPassword). Legacy users without firstName see a prompt in Settings to add their name. The dashboard shows a time-based greeting "Good morning/afternoon/evening, {firstName}" when firstName is set.

**Note:** Standalone 7-day meal plan and workout plan features have been removed from the frontend. All plan creation now flows through Wellness Plans (Goal Plans) or Daily Planning. Backward-compatible read-only shim routes exist for mobile/API clients: `GET /api/plan/:id`, `GET /api/workout/:id`, `GET /api/plan/:id/grocery`, `GET /api/plans`, `GET /api/workouts`. All swap, regeneration, and allowance/budget endpoints have been intentionally removed for launch simplification.

**Goal Taxonomy (v2)**: The goal architecture uses a two-field system. `primaryGoal` (on `user_profiles`) accepts: `weight_loss | muscle_gain | body_recomposition | general_fitness | athletic_performance`. `secondaryFocus` (on `user_profiles`, nullable) accepts: `strength | endurance | mobility | energy_focus | recovery`. Legacy values (`performance`, `maintenance`, `energy`, `mobility`, `endurance`, `strength`) are normalized at write time in routes and via a one-time SQL migration run on existing rows. Context-builder (`server/context-builder.ts`) threads `secondaryFocus` into both meal and workout prompt blocks. Wellness context (`server/wellness-context.ts`) uses it to compute macro targets and training guidance blocks per goal type. All frontend goal selectors (profile page, goal wizard, goal-plans list, goal-detail, sidebar) have been updated to use the new five canonical goal labels.
