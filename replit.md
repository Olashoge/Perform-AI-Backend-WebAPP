...
**Note:** Standalone 7-day meal plan and workout plan features have been removed from the frontend. All plan creation now flows through Wellness Plans (Goal Plans) or Daily Planning. However, backward-compatible API shim routes exist for mobile/API clients: `GET /api/plan/:id`, `GET /api/workout/:id`, `GET /api/plan/:id/grocery`, `POST /api/plan/:id/swap`, `POST /api/plan/:id/regenerate-day`, `POST /api/workout/:id/swap`, `POST /api/plan/:id/grocery/regenerate`, `GET /api/plans`, `GET /api/workouts`, `GET /api/allowance/current`. These resolve the parent goal plan and operate on nested child plan data.
